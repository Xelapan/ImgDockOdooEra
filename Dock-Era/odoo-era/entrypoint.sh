...
ADDON_PATH="/mnt/extra-addons/odoo16estrada_rodriguez"
BRANCH="main"
SECRETS_DIR="/var/lib/odoo/secrets"
TOKEN_FILE="$SECRETS_DIR/github_token"

# Rutas esperadas en el host (volúmenes)
HOST_SECRET_PATH="./OdooEra/secrets/github_token"
HOST_CONF_PATH="./OdooEra/odoo.conf"
#HOST_FILESTORE_PATH="./OdooTer/filestore"

echo "📦 Módulos Enterprise disponibles en: $ENTERPRISE_PATH"
echo "📦 Módulos odoo16finanzas se clonaran de github.com/Xelapan/odoo16estrada_rodriguez.git"
echo "Debe crear archivo github_token en ./OdooEra/secrets/ pegue en el archivo el token del repositorio"
echo "Debe crear archivo odoo.conf en ./OdooEraConf con la configuracion minima para odoo "
echo ""
...
if [ -z "$GITHUB_TOKEN" ]; then
  echo "❌ El archivo github_token está vacío. Agrega tu token de GitHub. en ./OdooEra/secrets/github_token"
  exit 1
fi

AUTH_REPO_URL="https://${GITHUB_TOKEN}@github.com/Xelapan/odoo16estrada_rodriguez.git"
